...
async function fbdl(url) {
  try {
    ...
    const thumbnail = $(
      "section > div:nth-child(2) > .results-item > div > img"
    ).attr("src");
    const high = $(
      "section > div:nth-child(2) > .results-download > ul > li:nth-child(1) > a"
    ).attr("href");
    const low = $(
      "section > div:nth-child(2) > .results-download > ul > li:nth-child(2) > a"
    ).attr("href");
  ...
  try {
    ...
    if (
      $(
        ...
        .attr("href")
        .includes("force-save.php")
    ) {
      var urll = decodeURIComponent(
        $(
          ".download-link > div:nth-child(2) > div > table > tbody > tr:nth-child(1) > td:nth-child(3) > a"
        )
          .attr("href")
...
async function igStory(username) {
  try {
    const { data } = await axios.get(
      `https://igram.world/api/ig/story?url=https://instagram.com/stories/${username}`,
      {
        headers: {
...
async function igStory2(username) {
  try {
    ...
    const { data } = await axios.get(
      `https://snapinst.com/api/ig/stories/${pkId}`
    );
    let result = { status: true, media: [] };
    data.result.map((check) => {
      if (check.has_audio === true) {
